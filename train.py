...
def run(rank, n_gpus, hps):
  ...
  if hps.wandb:
    wandb.login()
    wandb.init(project='fesde-text', name=hps.run_name)
  ...
  if hps.model.use_phoneme_predictor:
  ...
  optim_eeg = torch.optim.AdamW(
      list(eeg_module.parameters())+list(net_p.parameters()), 
      hps.train.learning_rate, 
      betas=hps.train.betas, 
      eps=hps.train.eps)
  ...
  if True: # continue from past run
    try:
      ...
      _, _, _, epoch_str = utils.load_checkpoint(utils.latest_checkpoint_path(hps.model_dir, "P_*.pth"), net_p, None)
  ...
  for epoch in range(epoch_str, hps.train.epochs + 1):
    if rank==0:
      train_and_evaluate(rank, epoch, hps, [net_g, net_d, eeg_module, net_p], [optim_g, optim_d, optim_eeg], [scheduler_g, scheduler_d, scheduler_eeg], scaler, [train_loader, eval_loaders], logger, [writer, writer_eval])
    else:
      train_and_evaluate(rank, epoch, hps, [net_g, net_d, eeg_module, net_p], [optim_g, optim_d, optim_eeg], [scheduler_g, scheduler_d, scheduler_eeg], scaler, [train_loader, None], None, None)
    scheduler_g.step()
    scheduler_d.step()
    scheduler_eeg.step()
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
  net_g, net_d, eeg_module, net_p = nets
  ...
  for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, phoneme, phoneme_lengths) in enumerate(train_loader):
    x, x_lengths = x.cuda(rank, non_blocking=True), x_lengths.cuda(rank, non_blocking=True)
    spec, spec_lengths = spec.cuda(rank, non_blocking=True), spec_lengths.cuda(rank, non_blocking=True)
    y, y_lengths = y.cuda(rank, non_blocking=True), y_lengths.cuda(rank, non_blocking=True)
    phoneme, phoneme_lengths = phoneme.cuda(rank, non_blocking=True), phoneme_lengths.cuda(rank, non_blocking=True)
    ...
    with autocast(enabled=False):
      ...
      loss_ctc = torch.nn.CTCLoss().cuda()(phoneme_preds.transpose(0,1), phoneme[:,1:], phoneme_lengths-1, phoneme_lengths-1)
    ...
    if rank==0:
      if global_step % hps.train.log_interval == 0:
        ...
        scalar_dict = {"loss/g/total": loss_gen_all, "loss/d/total": loss_disc_all, "learning_rate": lr, "grad_norm_d": grad_norm_d, "grad_norm_g": grad_norm_g, "loss/eeg/total":loss_eeg_module ,"loss/ctc/total":loss_ctc
                       }
      ...
      if global_step % hps.train.eval_interval == 0:
        for val_type, eval_loader in list(zip(['both', 'audio', 'subject'], loaders[1])):
          evaluate(hps, [net_g, eeg_module, net_p], eval_loader, writer_eval, val_type)
        ...
        utils.save_checkpoint(net_p, optim_eeg, hps.train.learning_rate, epoch, os.path.join(hps.model_dir, "P_{}.pth".format(global_step)))
    global_step += 1
...
def evaluate(hps, net_infer, eval_loader, writer_eval, val_type):
    net_g, eeg_module, net_p = net_infer
    net_g.eval()
    eeg_module.eval()

    with torch.no_grad():
      for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, phoneme, phoneme_lengths) in enumerate(eval_loader):
      ...
      y_hat, _, mask, *_ = net_g.module.infer(mid_output, mid_output_lengths, max_len=1000)
